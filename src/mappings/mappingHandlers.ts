...
// import {
//   isEraEvent,
//   isBridgeTransferEvent,
//   isBridgeTransferEventOld,
// } from './utils/matches'
...
//   const newCalls: Call[] = extrinsicWraps.reduce(
//     (cs, { newCalls }) => [...cs, ...newCalls],
//     []
//   )
//   const newEvents: Event[] = extrinsicWraps.reduce(
//     (es, { newEvents }) => [...es, ...newEvents],
//     []
//   )
//   const newSystemTokenTransfers: SystemTokenTransfer[] = extrinsicWraps.reduce(
//     (ss, { newSystemTokenTransfers }) => [...ss, ...newSystemTokenTransfers],
//     []
//   )
//   const newNearToAppchainTransfers: NearToAppchainTransfer[] =
//     extrinsicWraps.reduce(
//       (ss, { newNearToAppchainTransfers }) => [
//         ...ss,
...
//   await Promise.all(
//     Object.keys(accountIdMap).map(async (accountId) => {
//       const existedAccount = await Account.get(accountId)
//       if (existedAccount) {
//         await tryUpdateAccount(existedAccount, newBlock)
//       } else {
//         const handledAccount: Account = await handleAccount({
//           accountId,
//           block: newBlock,
//           creatorId: accountIdMap[accountId],
//         })
...
//   Promise.all([
//     store.bulkCreate('Account', newAccounts),
//     store.bulkCreate('Extrinsic', newExtrinsics),
//     store.bulkCreate('Call', newCalls),
//     store.bulkCreate('Event', newEvents),
//     ...newSystemTokenTransfers.map(async (data) => await data.save()),
//   ])

//   if (
//     block.block.header.number.toBigInt() >=
//     config.bridgeMessageStartAt.blockNumber
//   ) {
//     await Promise.all(
//       block.events.map(async (evt, idx) => {
//         if (
//           evt.event.section === 'octopusUpwardMessages' &&
//           evt.event.method === 'Committed'
//         ) {
//           newUpwardMessages.push(...handleUpwardMessages(block, evt))
//         }
//         if (
//           isEraEvent(evt.event) ||
//           isBridgeTransferEventOld(evt.event) ||
//           isBridgeTransferEvent(evt.event)
//         ) {
//           await storeBridgeMessageEvent(block, wExtrinsics, evt)
//         }
//       })
//     )
//     Promise.all(newUpwardMessages.map(async (data) => await data.save()))
//   }

//   Promise.all(newNearToAppchainTransfers.map(async (data) => await data.save()))
// }
