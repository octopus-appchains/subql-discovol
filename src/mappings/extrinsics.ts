...
import {
  handleAppchainToNearAccount,
  handleNearToAppchainTransfer,
} from './bridgeEvents'
...
export function handleExtrinsic(
  ...
  extrinsic.events.forEach((evt, idx) => {
    newEvents.push(
      handleEvent(block, extrinsic, evt, extrinsicId, startEvtIdx + idx)
    )
    if (evt.event.section === 'balances' && evt.event.method === 'Transfer') {
      newSystemTokenTransfers.push(
        handleSystemTokenTransfer(
          block,
          extrinsic,
          evt,
          extrinsicId,
    ...
    if (evt.event.section === 'octopusBridge') {
      if (
        ['Locked', 'Nep141Burned', 'NonfungibleLocked'].includes(
          evt.event.method
        )
      ) {
      ...
      if (
        ['Unlocked', 'Nep141Minted', 'NonfungibleUnlocked'].includes(
          evt.event.method
        )
      ) {
        newNearToAppchainTransfers.push(
          handleNearToAppchainTransfer(
            block,
            extrinsic,
            evt.event,
